...
class Flyer:
    ...
    async def _request(self, method: str, params: dict = {}) -> dict:
        url = f'https://api.flyerhubs.com/{method}'
    ...
    async def check(self,
        ...
        if 'skip' not in result and 'error' in result:
            raise APIError('user_id={} language_code={}, error={}'.format(
    ...
    async def check_task(self, signature: str) -> Union[str, None]:
        """
        ...
        # Calling an exception
        if 'result' not in result and 'error' in result:
            raise APIError('signature={}, error={}'.format(
                signature,
                result['error'],
            ))
    ...
    async def get_tasks_max(self,
        ...
        :param user_id: User ID
        :param user_locale: Language code of user ID
        :param limit: The maximum number of resources to search for
        ...
        if not isinstance(chat_id, int):
            logger.error('chat_id must be an integer, got {}'.format(type(chat_id).__name__))
            return []
        if not isinstance(user_id, int):
            logger.error('user_id must be an integer, got {}'.format(type(user_id).__name__))
            return []
        ...
        # Calling an exception
        if 'result' not in result and 'error' in result:
            raise APIError('chat_id={} user_id={} user_locale={}, error={}'.format(
                chat_id, user_id, user_locale,
                result['error'],
            ))
    ...
    async def check_task_max(self, signature: str) -> Union[str, None]:
        """
        Getting the result of completing the task.
    ...
    async def get_completed_tasks(self, user_id: int) -> Union[dict, None]:
        """
        Getting completed tasks from the service.

        :param user_id: User ID

        :return: List of completed resources
        """
        ...
        # Calling an exception
        if 'result' not in result and 'error' in result:
            raise APIError('user_id={}, error={}'.format(
                user_id,
                result['error'],
            ))
